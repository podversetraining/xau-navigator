  ...
  return (
    <div className="flex flex-col items-center gap-1">
      <svg width={size} height={size} className="-rotate-90">
        <circle cx={size/2} cy={size/2} r={radius} fill="none" stroke="hsl(220, 15%, 12%)" strokeWidth={6} />
        <motion.circle
      ...
      <span className="font-display text-lg font-bold text-foreground" style={{ marginTop: -size/2 - 10, position: "relative" }}>
        {value.toFixed(1)}
      </span>
      <span className="text-dim text-xs font-data mt-4">{label}</span>
    </div>
  );
}

function IndicatorCard({ label, value, status }: { label: string; value: string; status: "bullish" | "bearish" | "neutral" }) {
  const borderClass = status === "bullish" ? "border-l-[hsl(142,72%,45%)]" : status === "bearish" ? "border-l-[hsl(0,72%,51%)]" : "border-l-[hsl(43,96%,56%)]";
  return (
    <div className={`bg-secondary rounded-md p-3 border-l-2 ${borderClass}`} style={{ borderLeftColor: status === "bullish" ? "hsl(142,72%,45%)" : status === "bearish" ? "hsl(0,72%,51%)" : "hsl(43,96%,56%)" }}>
  ...
  return (
    <div className="h-full flex flex-col gap-4 p-8">
      <div className="flex items-center justify-between">
        <h2 className="font-display text-lg tracking-widest text-dim">LAYER 1 — TREND ANALYSIS</h2>
        <div className="flex items-center gap-3">
          <span className={`font-display text-2xl font-bold ${trendColor}`}>{layer.trend.toUpperCase()}</span>
          <span className="text-gold font-display text-xl">{analysis.score.layer1}/40</span>
        </div>
      </div>

      <div className="grid grid-cols-4 gap-4 flex-1">
        {/* EMA Panel */}
        <div className="glass-panel rounded-lg p-4 gold-border-glow col-span-2">
          <h3 className="font-display text-xs tracking-widest text-gold mb-3">MOVING AVERAGES — D1</h3>
          {d1 && (
        ...
        <div className="glass-panel rounded-lg p-4 gold-border-glow">
          <h3 className="font-display text-xs tracking-widest text-gold mb-3">TREND INDICATORS</h3>
        ...
        {/* Ichimoku & Fibonacci */}
        <div className="glass-panel rounded-lg p-4 gold-border-glow">
          <h3 className="font-display text-xs tracking-widest text-gold mb-3">ICHIMOKU & FIB</h3>
          <div className="flex flex-col gap-2">
            {d1 && (
              <>
